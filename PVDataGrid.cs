...
namespace ProView
{
    internal partial class PVDataGrid : DataGridView
    {
        ...
        public PVDocument CurrentDocument
        {
            get
            {
                return _loadedRow?.Tag as PVDocument;
            }
        }

        public event EventHandler<DocumentChangingEventArgs> CurrentDocumentChanging;
        public event EventHandler<DocumentChangedEventArgs> CurrentDocumentChanged;
        ...
        public class DocumentChangingEventArgs : EventArgs
        {
            ...
            public DocumentChangingEventArgs(PVDocument document)
            {
                Document = document;
            }
        }
        ...
        public class DocumentChangedEventArgs : EventArgs
        {
            ...
            public DocumentChangedEventArgs(PVDocument document)
            {
                Document = document;
            }
        }

        protected void OnCurrentDocumentChanged(object sender, DocumentChangedEventArgs e)
        {
            var handler = CurrentDocumentChanged;
            handler?.Invoke(sender, e);
        }

        protected void OnCurrentDocumentChanging(object sender, DocumentChangingEventArgs e)
        {
            var handler = CurrentDocumentChanging;
            handler?.Invoke(sender, e);
        }

        DataGridViewRow _loadedRow;
        ...
        DataGridViewRow LoadedRow
        {
            ...
            set
            {
                if (value != _loadedRow)
                {
                    var e = new DocumentChangingEventArgs(CurrentDocument);
                    OnCurrentDocumentChanging(this, e);
                    ...
                    if (_loadedRow is not null)
                    {
                        _loadedRow.DefaultCellStyle.BackColor = DefaultCellStyle.BackColor;
                        _loadedRow.DefaultCellStyle.ForeColor = DefaultCellStyle.ForeColor;
                    }
                    ...
                    if (_loadedRow is not null)
                    {
                        _loadedRow.DefaultCellStyle.BackColor = Properties.DataGrid.Default.HighlightRowBG;
                        _loadedRow.DefaultCellStyle.ForeColor = Properties.DataGrid.Default.HighlightRowFG;
                    }

                    OnCurrentDocumentChanged(this, new DocumentChangedEventArgs(CurrentDocument));
                }
            }
        }
        ...
        public void AddItem(PVDocument pvFile)
        {
            // Check if the file has already been added
            foreach (DataGridViewRow row in Rows)
            {
                var existingFile = row.Tag as PVDocument;
        ...
        public void RenameFiles()
        {
            ...
            progressWindow = new PVProgressForm();
        ...
        void renameFilesBgWorker_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            ...
            {
                var pvFile = dgvr.Tag as PVDocument;
